...
This map shows precincts that Katie won (>50%) in blue and Katie did not win (<50%) in red. Saturation/transparency of the color indicates absolute current turnout as of `r today`.
```{r}
plt <- precinct_returns |>
  mutate(
    ...
    f_turnout = cut(
      obs_turnout,
      breaks = seq(0, ceiling(max(obs_turnout) * 10) / 10, length.out = 6)
    )
  ) |>
  ggplot(aes(
  ...
  geom_sf(linewidth = 0.1, aes(alpha = f_turnout)) +
  scale_fill_manual(values = c('red', 'blue')) +
  scale_alpha_discrete(range = c(0.15, 1)) +
  labs(fill = 'Primary supporter', alpha = 'Turnout')
...
plt <- precinct_returns |>
  mutate(
    ...
    f_pct_expected = cut(
      pct_of_expected,
      breaks = seq(0, ceiling(max(pct_of_expected) * 10) / 10, length.out = 6)
    )
  ) |>
  ggplot(aes(
    fill = primary_candidate,
